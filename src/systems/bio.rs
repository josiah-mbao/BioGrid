...
pub fn attraction_system(
    ...
    for (mut pos, mut energy) in query.iter_mut() {
        ...
        if distance_squared > 1 {
            ...
            let is_occupied = friend_positions
                .iter()
                .any(|&other_pos| other_pos == target_pos && other_pos != pos.0);
            ...
            if !is_occupied {
                ...
                for &adjacent_pos in &adjacent_positions {
                    let is_adjacent_occupied = friend_positions
                        .iter()
                        .any(|&other_pos| other_pos == adjacent_pos && adjacent_pos != pos.0);
        ...
        let plants_to_eat: Vec<(Entity, f32)> = plant_query
            .iter()
...
pub fn metabolism_system(mut commands: Commands, query: Query<(Entity, &Energy), With<Friend>>) {
